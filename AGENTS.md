...
Read `README.md`, `docs/status.md`, and the documents relevant to the task before editing. Check `git status --short --branch` and recent commits; preserve work from the user or another agent. These instructions are shared by Codex and Claude Code through `CLAUDE.md`.
...
- PETG is a candidate structural material. “Timemass glowing plant vitamin” is an unidentified working label. Exact composition, processing, and plant compatibility remain unknown.
...
Keep shared rules in this file; keep `CLAUDE.md` as a small import entry point. Put changing project state in `docs/status.md`, and detailed evidence or decisions in dated build-log entries. Do not rely on a chat transcript or one tool's private memory for project-critical facts.
