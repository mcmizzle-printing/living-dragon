...
Keep shared rules in `.context/AGENTS.md`; preserve root `AGENTS.md` and `CLAUDE.md` as relative symlinks to it. Edit the canonical file instead of replacing either symlink. Put changing project state in `docs/status.md`, and detailed evidence or decisions in dated build-log entries. Do not rely on a chat transcript or one tool's private memory for project-critical facts.
